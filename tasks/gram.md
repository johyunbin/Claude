...
### 미완 항목 처리 결과 (2026-05-16 18:16 KST — 그램 로컬 세션)

- **Wi-Fi 드라이버** ✅ — 이미 최신. 현재 Intel `24.40.0.4` / **2026-04-13**자.
  (이전 기록 `22.70.0.6`/2021 → 사용자가 그 사이 갱신함.)
- **NIC 절전 해제** ✅ — `Get-NetAdapterPowerManagement` WMI 는 여전히 무응답.
  대신 레지스트리로 처리: `...\Class\{4d36e972-...}\0001\PnPCapabilities`
  미설정 → `256`(0x100, "전원 절약을 위해 끌 수 있음" 해제)으로 DWORD 생성.
  **다음 부팅 시 적용**(어댑터 재시작 안 함 — 무중단).
- **Windows 업데이트** ✅ — 보류분 점검함. Defender 시그니처 KB2267602 1건만
  대기(일상 정의 업데이트, 자동 설치됨). 실질 보류 없음.
- **유선(이더넷)** ✅ — 확인 결과 LG Gram 에 RJ45 포트 없음. 물리 어댑터는
  Wi-Fi(AX201) 뿐(나머지는 VMware/ZeroTier/Tailscale 가상). 유선 쓰려면
  USB-C 이더넷 동글 필요 — 하드웨어 구매 영역.
- **덮개 닫기 동작** ✅ — `tasks/gram-remote-access.md` 에서 AC+DC 둘 다
  "아무것도 안 함"으로 설정 완료.
